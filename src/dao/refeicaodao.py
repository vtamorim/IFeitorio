...
class RefeicaoDAO(AbstractDAO):
    ...
    def add(cls, obj: Refeicao) -> None:
        ...
        sql_code = "INSERT INTO refeicao (id, nome, descricao) VALUES (?, ?, ?)"
        cursor.execute(sql_code, (obj.get_id(), obj.get_nome(), obj.get_descricao()))
    ...
    def get_all(cls) -> list[Refeicao]:
        ...
        return [
            Refeicao(row.id, row.nome, row.descricao, row.get(row.id, []))
            for row in rows
        ]
    ...
    def update(cls, new_obj: Refeicao) -> None:
        ...
        sql_code = "UPDATE refeicao SET nome = ?, descricao = ? WHERE id = ?"
